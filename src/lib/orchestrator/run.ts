...
import { writeReport, type PricedInJudgment } from "./write-report";
...
export type ResearchCommand = {
  ...
  /**
   * How agents should work this command. Never "search the inventory."
   * Infer situations that would create need for what the business sells,
   * then search those situations. Agents self-select; decline is free.
   */
  method?: string;
  /**
   * Concrete lookups from the orchestrator: "look up X, look up Y".
   * Situation-shaped (builds, tenders, permits), never the inventory keyword.
   * Agents should run these first, then add their own beat if they have room.
   */
  search_hints?: string[];
...
/**
 * Shared method line on every dispatch. Domain-agnostic: the business may
 * sell anything; agents infer demand-creating situations, never a product
 * keyword checklist. Grid has no taxonomy — every online agent gets this.
 */
export const AGENT_METHOD = [
  "Do NOT search for who buys the inventory as a product keyword.",
  "USE THE ORCHESTRATOR search_hints FIRST — those are the lookups to run.",
  "If hints are thin, infer situations that would create need for what this business sells, then search those.",
  "Partial observations are valid: site, street, builder, contact.",
  "Use the full window to pool findings. If this is not your market, decline — silence and declines are free.",
  "Every agent on the grid receives this same command. You decide whether to answer.",
].join(" ");

/**
 * Wave-one situation lookups. Domain-agnostic: derived from geography +
 * demand-creating event types, never from a product checklist. LLM
 * hypotheses replace these on wave two with observation-grounded hints.
 */
export function orchestratorSearchHints(
  question: string,
  scope: { category?: string; geography?: string },
): string[] {
  const geo = scope.geography?.trim() || "";
  const situations = [
    "construction expansion new facility",
    "groundbreaking building permit",
    "tender procurement RFP award",
    "capacity expansion plant opening",
    "refurbishment renovation contract",
    "hospital hotel clinic school opening",
  ];
  const hints: string[] = [];
  let i = 0;
  while (i < situations.length && hints.length < 6) {
    const s = situations[i]!;
    hints.push(geo ? `${s} ${geo}` : s);
    i += 1;
  }
  // Keep a clip of the raw question so agents still see the business context
  // without being told to search it as a keyword.
  void question;
  return hints;
}
...
export function isPullEndpoint(endpoint: string): boolean {
  const e = endpoint.trim().toLowerCase();
  return e === "pull" || e === "poll" || e.startsWith("pull://");
}

async function dispatchToAgent(endpoint: string, command: ResearchCommand): Promise<boolean> {
  if (isPullEndpoint(endpoint)) return true;
  try {
...
async function dispatchWave(params: {
  ...
  const base = {
    ...
    method: AGENT_METHOD,
    search_hints: (() => {
      const fromHypotheses = (params.hypotheses ?? [])
        .flatMap((h) => h.searchHints ?? [])
        .filter(Boolean)
        .slice(0, 8);
      if (fromHypotheses.length >= 2) return fromHypotheses;
      return orchestratorSearchHints(params.question, params.scope);
    })(),
  ...
  const results = await Promise.allSettled(
    dispatched.map(async (agent) => {
      ...
      // Always enqueue so pull-mode agents can fetch the command.
      await db.insert(commandOutbox).values({
        commandId: cmd.command_id,
        inquiryId: params.inquiryId,
        agentId: agent.id,
        payloadJson: JSON.stringify(cmd),
        status: "pending",
        createdAt: nowIso(),
      });
  ...
  results.forEach((r, i) => {
    ...
    // Pull agents stay online; only push endpoints can go offline on failed POST.
    if (isPullEndpoint(agent.endpoint)) {
      void db.update(agents).set({ lastSeen: nowIso() }).where(eq(agents.id, agent.id));
      return;
    }
    void db
      .update(agents)
...
export async function gradeAndSynthesize(inquiryId: string) {
  ...
  try {
    ...
    try {
      // Market check removed — chains stand on the evidence alone.
      ...
      // No ticker market check: priced-in is always unclear (honest empty).
      const pricedIn: PricedInJudgment = {
        verdict: "unclear",
        reason: "Market check is not part of this product; assessment stands on the evidence chain alone.",
        reactions: [],
      };
      ...
      // Agent settlement only: copy the final package for contribution
      // matching, then adjudicate weights. The business already has this
      // report — GenLayer never gates the human path.
      void (async () => {
        try {
          const { recordFinal, adjudicate, judgeAddress } = await import("@/lib/genlayer/judge");
          const { packageFinalIntelligence } = await import("@/lib/genlayer/contribution");
          const finalPkg = packageFinalIntelligence({
            question: inquiry.question,
            opportunities: readout.map((r) => ({
              company: r.company,
              location: null,
              need: r.topClaim,
              summary: r.topClaim,
          ...
          const ok = await recordFinal({ inquiryId, finalIntelligence: finalPkg });
          console.log(`[genlayer] record_final ${ok ? "ok" : "failed"} @ ${judgeAddress()}`);
          if (ok) {
            const weights = await adjudicate(inquiryId);
            console.log(
              "[genlayer] adjudicate:",
              weights ? JSON.stringify(weights).slice(0, 200) : "no weights yet",
            );
          }
        } catch (err) {
          console.error("genlayer post-report failed:", err);
        }
      })();
    } catch (err) {
