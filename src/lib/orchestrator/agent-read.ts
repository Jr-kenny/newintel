...
/** Topic match against company names — no ticker market map. */
function companyMentions(company: string, want: string): boolean {
  return company.toLowerCase().includes(want.toLowerCase());
}
...
async function listRuns(ticker: string): Promise<MatchedRun[]> {
  ...
  for (const c of candidates) {
    if (c.row.question.toLowerCase().includes(want.toLowerCase())) {
      direct.push(toRun(c));
      continue;
    }
    const hit =
      (c.synthesis?.recommendations.some((r) => companyMentions(r.company, want)) ?? false) ||
      (c.report?.evidence.some((e) => companyMentions(e.entity, want)) ?? false) ||
      c.readout.some((e) => companyMentions(e.company, want));
...
export type InvestigationStart =
  | {
      ...
      inquiryId: string;
      windowSeconds: number;
      /** Poll newintel_inquiry this often until status is complete. */
...
export async function agentInvestigate(question: string): Promise<InvestigationStart> {
  ...
  const submitUrl = process.env["PUBLIC_SUBMIT_URL"]
