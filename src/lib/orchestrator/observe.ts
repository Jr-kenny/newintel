...
async function fetchHeadlines(query: string): Promise<ObservedHeadline[]> {
  ...
  const res = await fetch(url, {
    headers: { "user-agent": "Mozilla/5.0 (compatible; Newintel-Observe/1.0)" },
 ...
 * happening now, not from recollection. Best effort: fresh headlines and
 * prior run state. Failure leaves that leg empty, never blocks dispatch.
 */
export async function observeQuestion(
  question: string,
  recall: RecallResult,
): Promise<Observation> {
  // No ticker market check — probe is headlines + memory only.
  const topics = topicWords(question);
  const query = topics.join(" ") || question.trim().slice(0, 80);
  ...
  const lines: string[] = [];
  lines.push(`TOPIC: ${query || question.slice(0, 60)}`);
  ...
  return { ticker: null, priceLine: "", headlines, memoryNote, text: lines.join("\n") };
}
