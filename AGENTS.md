# Newintel — Agent Guide

Port of Runintel for equities (Track A: Binance Agent OS Mini Hackathon).
Event → exposure → thesis → Binance market context. `bun run dev` on 8080, `bun run build` for prod.
UI direction: isometric city — buildings = watched tickers / exposure nodes, hover = lift + thesis.


## WRITEUP GUIDES BOTH ON THE UI AND ALSO FOR ANYWORK
