...
type ResearchCommand = {
  ...
  /** Orchestrator lookups — run these first, not the inventory keyword. */
  search_hints?: string[];
  method?: string;
...
function buildQueries(cmd: ResearchCommand): string[] {
  const out: string[] = [];
  // Orchestrator lookups first ("look up this, look up that").
  const flatHints = (cmd.search_hints ?? []).filter(Boolean).slice(0, 6);
  if (flatHints.length >= 2) {
    const geo = cmd.scope.geography ? ` ${cmd.scope.geography}` : "";
    out.push(...flatHints.map((q, i) => aim(`${q}${geo}`.trim(), i)));
  }
  // Then hypothesis searchHints if the orchestrator sent them (wave two).
  if (out.length === 0 && cmd.hypotheses?.length) {
